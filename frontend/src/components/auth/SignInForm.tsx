...
export default function SignInForm() {
  ...
  const { mutate, error, isSuccess, isPending } = useUserLogin();
  const token = sessionStorage.getItem("token");
  const router = useRouter();
  console.log("GET TOKEN IN SIGN IN:", token);
  const user = useSelector((state: RootState) => state.auth.user);

  console.log("GET user IN SIGN IN:", user);
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                {/* <div>
                  ...
                  <Input onChange={(e) => setEmail(e.target.value)} value={email} placeholder="info@gmail.com" type="email"/>
                </div> */}
